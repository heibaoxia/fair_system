...
class ProjectDetailDependencyAutosaveTests(unittest.TestCase):
    ...
    def test_dependency_autosave_requests_use_session_routes_without_current_member_id(self):
        content = Path("app/templates/project_detail.html").read_text(encoding="utf-8")

        self.assertIn("await fetch(`/projects/${projectId}/dependencies`, {", content)
        self.assertIn("await fetch(`/projects/${projectId}/dependencies/${persistedConnection.id}`, {", content)
        self.assertNotIn("current_member_id", content)
